...
const loginSchema = z.object({
	...
	password: z.string({ required_error: 'Campo requerido' }).min(5, { message: 'Campo requerido' })
});
...
export const actions: Actions = {
	login: async ({ request, cookies }) => {
		...
		if (formData.success) {
			...
			const passwordMatches = await bcryptjs.compare(formData.data.password, existingUser.password);
			if (!passwordMatches)
				return fail(400, {
					containsErrors: true,
					fields: {
		...
		const containsErrors = Boolean(
			formData.error.formErrors.fieldErrors.email || formData.error.formErrors.fieldErrors.password
		);
